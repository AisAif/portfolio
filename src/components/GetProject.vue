...
<template>
    <div>
        <div class="h-screen md:px-6 grid grid-cols-1 sm:grid-cols-2 gap-3 justify-items-center overflow-y-auto no-scrollbar">
            <div class="flex flex-col w-fit shadow-xl dark:border-2 dark:border-accent-light rounded-lg" v-for="project in projects" :key="project.id">
                ...
                <transition name="fade-size" mode="in-out">
                    <div class="absolute left-0 top-0 w-full h-full flex flex-col justify-center items-center backdrop-blur-sm z-50" v-if="project.id == projectIdOpen">
                        <div class="relative flex flex-col justify-center items-center w-[340px] sm:w-[600px] rounded p-4 sm:p-8 bg-accent-dark dark:bg-accent-light text-slate-200">
                            ...
                            <a :href="project.link" target="_blank" class="bg-accent-light dark:bg-accent-dark hover:bg-secondary hover:text-accent-dark dark:hover:text-accent-light p-2 rounded mt-5">Hasil atau Source Code</a>
                        </div>
                    </div>
                </transition>
            </div>
